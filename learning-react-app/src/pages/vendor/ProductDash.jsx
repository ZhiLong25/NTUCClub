...
function ProductDash() {
  ...
  useEffect(() => {
    ...
    http.get('/user/auth')
      .then((res) => {
        ...
        setVendorname(res.data.user);
          console.log(res.data.user.name); // Now this will have the correct value
        http.get(`/Product/getvendorservice/${res.data.user.name}`)
          .then((res) => {
  ...
  return (
    <Box className="main-wrap admin-wrap" >
      <Typography variant="h5" className="main-title" style={{ marginTop: "20px", marginBottom: "10px" }}>Welcome Vendor {name.name}</Typography>
